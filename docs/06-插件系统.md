...
interface BasePluginType<T, C, Class> {
  name: T;                    // 插件名称（事件名）
  type: Class;               // 插件类型分类
  monitor: (emit: Function) => void;  // 监听器函数
...
const jsErrorPlugin: BasePluginType<BrowserErrorTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    window.addEventListener('error', (event) => {
      ...
      emit(BrowserErrorTypes.JE, event);
    }, true);
  },
  ...
  monitor(emit) {
    ...
    const observer = new PerformanceObserver((list) => {
      ...
      for (const entry of list.getEntries()) {
        ...
        emit(BrowserPerformanceTypes.LCP, reportData);
      }
    });
...
const customPlugin: BasePluginType<CustomEventType, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    window.addEventListener('custom-event', (event) => {
      emit(CustomEventType.CUSTOM_EVENT, event.detail);
    });
  },
...
monitor(emit) {
  try {
    ...
    window.addEventListener('some-event', (event) => {
      emit(EventType.SOME_EVENT, event);
    });
  } catch (error) {
...
monitor(emit) {
  ...
  const throttledNotify = throttle((data) => {
    emit(EventType.SOME_EVENT, data);
  }, 100);
