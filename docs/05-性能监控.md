    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.entryType === 'largest-contentful-paint') {
          ...
          emit(BrowserPerformanceTypes.LCP, lcpData);
        }
      }
    });
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.entryType === 'first-input') {
          ...
          emit(BrowserPerformanceTypes.FID, fidData);
        }
      }
    });
...
const clsPlugin: BasePluginType<BrowserPerformanceTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        emit(BrowserPerformanceTypes.CLS, {
          value: clsValue,
          entries: clsEntries
        });
      }
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.name === 'first-paint') {
          emit(BrowserPerformanceTypes.FP, {
            value: entry.startTime,
            timestamp: Date.now()
          });
        }
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.name === 'first-contentful-paint') {
          emit(BrowserPerformanceTypes.FCP, {
            value: entry.startTime,
            timestamp: Date.now()
          });
        }
...
const fmpPlugin: BasePluginType<BrowserPerformanceTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    function calculateFMP() {
      ...
      isCalculated = true;
      emit(BrowserPerformanceTypes.FMP, {
        value: fmpTime,
        timestamp: Date.now()
      });
    }
...
const navigationPlugin: BasePluginType<BrowserPerformanceTypes, BrowserClient> = {
  ...
  monitor(emit) {
    function collectNavigationTiming() {
      ...
      emit(BrowserPerformanceTypes.NT, timing);
    }
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.entryType === 'resource') {
          ...
          emit(BrowserPerformanceTypes.RT, resourceData);
        }
      }
    });
...
const memoryPlugin: BasePluginType<BrowserPerformanceTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    function collectMemoryInfo() {
      ...
      emit(BrowserPerformanceTypes.MEMORY, memoryInfo);
    }
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.entryType === 'longtask') {
          ...
          emit(BrowserPerformanceTypes.LONG_TASK, longTaskData);
        }
      }
    });
