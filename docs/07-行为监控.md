...
const pvPlugin: BasePluginType<BrowserBehaviorTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    const recordPV = () => {
      ...
      emit(BrowserBehaviorTypes.PV, pvData);
    };
...
const clickPlugin: BasePluginType<BrowserBehaviorTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    document.addEventListener('click', (event) => {
      ...
      emit(BrowserBehaviorTypes.CLICK, clickData);
    });
  },
...
const hashRoutePlugin: BasePluginType<BrowserBehaviorTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    window.addEventListener('hashchange', () => {
      ...
      emit(BrowserBehaviorTypes.HASH_ROUTE, routeData);
      from = to;
    });
  },
  ...
  monitor(emit) {
    ...
    function wrapStateMethod(method: any) {
      return function(...args: any[]) {
        ...
        emit(BrowserBehaviorTypes.HISTORY_ROUTE, routeData);
        from = to;
  ...
  monitor(emit) {
    ...
    const throttledScroll = throttle(() => {
      ...
      this.scrollTimeout = setTimeout(() => {
        ...
        emit(BrowserBehaviorTypes.SCROLL, scrollData);
        isScrolling = false;
      }, 150);
    }, 100);
...
const formPlugin: BasePluginType<BrowserBehaviorTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    document.addEventListener('submit', (event) => {
      ...
      emit(BrowserBehaviorTypes.FORM, submitData);
    });
    ...
    document.addEventListener('focusin', (event) => {
      ...
      if (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || target.tagName === 'SELECT') {
        ...
        emit(BrowserBehaviorTypes.FORM, focusData);
      }
    });
  },
...
const devicePlugin: BasePluginType<BrowserBehaviorTypes, BrowserClient> = {
  ...
  monitor(emit) {
    ...
    window.addEventListener('orientationchange', () => {
      ...
      emit(BrowserBehaviorTypes.DEVICE, deviceData);
    });
    ...
    window.addEventListener('resize', throttle(() => {
      ...
      emit(BrowserBehaviorTypes.DEVICE, resizeData);
    }, 300));
    
    // 网络状态变化
    window.addEventListener('online', () => {
      emit(BrowserBehaviorTypes.DEVICE, {
        type: MonitorClassTypes.behavior,
        subType: BrowserBehaviorTypes.DEVICE,
        pageURL: getPageUrl(),
        time: getTimestamp(),
