...
window.addEventListener('error', (event: ErrorEvent) => {
  ...
  // 发布错误事件
  emit(BrowserErrorTypes.JE, errorInfo);
}, true);
```
...
window.addEventListener('error', (event: ErrorEvent) => {
  ...
  emit(BrowserErrorTypes.RE, resourceError);
}, true);
```
...
window.addEventListener('unhandledrejection', (event: PromiseRejectionEvent) => {
  ...
  emit(BrowserErrorTypes.PE, promiseError);
});
```
...
console.error = function(...args: any[]) {
  ...
  emit(BrowserErrorTypes.CE, consoleError);
};
```
...
const vuePlugin: VuePlugin = {
  install(Vue: VueConstructor) {
    Vue.config.errorHandler = (error: Error, vm: Vue, info: string) => {
      ...
      emit(BrowserErrorTypes.VUE, vueError);
    };
  }
};
```
...
class ErrorBoundary extends React.Component {
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    emit(BrowserErrorTypes.REACT, reactError);
  }
